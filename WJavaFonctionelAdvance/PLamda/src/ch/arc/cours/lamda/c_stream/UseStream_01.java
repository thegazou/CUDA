...
public class UseStream_01
	...
	private static void useMaptoInt(int n)
		...
		// lamda dans varibale, reference methode si possible
			...
			Integer sum = list.stream().mapToInt(age).sum(); // mapToInt renvoie un stream de Int qui possède la méthode sum()
			OptionalDouble average = list.stream().mapToInt(hauteur).average();
			IntSummaryStatistics stat = list.stream().mapToInt(age).summaryStatistics();
		...
		// lamda, reference methode si possible
			...
			IntSummaryStatistics stat = list.stream().mapToInt(Personne::getAge).summaryStatistics();
