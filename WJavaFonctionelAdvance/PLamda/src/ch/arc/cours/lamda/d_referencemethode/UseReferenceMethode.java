...
public class UseReferenceMethode
	...
	private static void methode0()
		...
		// lamda
			...
			Integer sumPaire = Arrays.stream(tab).parallel().filter(x -> IntegerTools.isPaire(x)).reduce(0, (x, y) -> x + y); // Integer.sum(x,y)
	...
	private static void methode1()
		...
		// lamda
			...
			Integer prixTotal = Arrays.stream(tab).parallel().mapToInt(maison -> maison.getPrix()).sum();
		...
		//		prix
			...
			Integer prixTotal = Arrays.stream(tab).parallel().mapToInt(Maison::getPrix).sum();
	...
	private static void methode2()
		...
		// lamda  reference methode
			...
			Arrays.parallelSort(tab, String::compareToIgnoreCase); //Piège
	...
	private static void constructeur1()
		...
		// lamda
			...
			try
				...
				List<Number> list = Arrays.stream(tab).parallel().map(s -> new Integer(s)).collect(Collectors.toList());
		...
		// lamda , reference methode
			...
			try
				...
				List<Number> list = Arrays.stream(tab).parallel().map(Integer::new).collect(Collectors.toList());
	...
	private static void constructeur3()
		...
		// lamda sans variable, avec reference de methode
			{
			JButton[] tab = Arrays.stream(tabLabel).parallel().map(JButton::new).toArray(JButton[]::new);
	...
	private static void arraytoList()
		...
		// avec stream
			{
			List<Integer> list = Arrays.stream(tab).parallel().collect(Collectors.toList());
	...
	private static void listToArray()
		...
		// lamda dans variable, sans reference de methode
			...
			Integer[] tab = list.parallelStream().toArray(generator);
		...
		// lamda sans variable, avec avec reference de methode
			{
			Integer[] tab = list.parallelStream().toArray(Integer[]::new);
